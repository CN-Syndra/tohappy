...
- [Authenticated connections](#authentication); authenticated client connections with a users config file, authenticated server connections with finger matching.
...
$ chisel server --help
  ...
  Options:
    ...
    key pair. Share the subsequent finger with clients to enable detection
...
$ chisel client --help
  ...
  Options:
    ...
    --finger, A *strongly recommended* finger string
    to perform host-key validation against the server's public key.
	Finger mismatches will close the connection.
	Finger are generated by hashing the ECDSA public key using
	SHA256 and encoding the result in base64.
	Finger must be 44 characters containing a trailing equals (=).
    ...
    may be still verified (see --finger) after inner connection
    is established.
...
Encryption is always enabled. When you start up a chisel server, it will generate an in-memory ECDSA public/private key pair. The public key finger (base64 encoded SHA256) will be displayed as the server starts. Instead of generating a random key, the server may optionally specify a key seed, using the `--key` option, which will be used to seed the key generation. When clients connect, they will also display the server's public key finger. The client can force a particular finger using the `--finger` option. See the `--help` above for more information.
...
2. Connect your chisel client (using server's finger)

```sh
chisel client --finger 'Yndjqpryzbfmpr+siweo3YweoPhfb6qw93VsoMweri2=' <server-address>:9312 socks
```
