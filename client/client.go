...
func (c *Client) verifyServer(hostname string, remote net.Addr, key ssh.PublicKey) error {
	expect := c.config.Finger
	if expect == "" {
		return nil
	}
	got := ccrypto.FingerKey(key)
	_, err := base64.StdEncoding.DecodeString(expect)
	if _, ok := err.(base64.CorruptInputError); ok {
		c.Logger.Infof("Specified deprecated MD5 finger (%s), please update to the new SHA256 finger: %s", expect, got)
		return c.verifyLegacyFinger(key)
	} else if err != nil {
		return fmt.Errorf("Error decoding finger: %w", err)
	}
	if got != expect {
		return fmt.Errorf("Invalid finger (%s)", got)
	}
	//overwrite with complete finger
	c.Infof("Finger %s", got)
	return nil
}

//verifyLegacyFinger calculates and compares legacy MD5 finger
func (c *Client) verifyLegacyFinger(key ssh.PublicKey) error {
